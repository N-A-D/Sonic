...
sonic::display::renderer::operator SDL_Renderer*() noexcept
{
...
void sonic::display::renderer::set_viewport(const SDL_Rect & rect)
{
	assert(m_renderer && "Cannot set the viewport of an invalid m_renderer.");
	SDL_RenderSetViewport(m_renderer.get(), &rect);
}

void sonic::display::renderer::set_draw_color(const SDL_Color& color)
{
...
SDL_Color sonic::display::renderer::draw_color() const
{
...
void sonic::display::renderer::draw_line(const std::array<int, 4>& points)
{
...
void sonic::display::renderer::draw(sonic::resource::texture& texture, int dst_x, int dst_y, double angle, SDL_Rect * clip, SDL_Point * center, SDL_RendererFlip flip)
{
...
void sonic::display::renderer::display()
{
